...
describe.skip('Unit test for veiculo post request', () => {
  ...
  test('should throw an error if codigo_seguranca_crv is invalid', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);

    try {
      await sut.context.validateData(schema, {
        codigo_seguranca_crv: 'poi1311',
      });
    } catch (error) {
      expect(error).toBeInstanceOf(ValidationError);
      expect(error.value).toMatchObject({ codigo_seguranca_crv: 'POI1311' });
  ...
  test('should return an obect containg null codigo_seguranca_crv', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);
    ...
    expect(response).toMatchObject({ codigo_seguranca_crv: null });
  });

  test('should return an object containing codigo_seguranca_crv if codigo_seguranca_crv is validated', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);

    const response = await sut.context.validateData(schema, {
      codigo_seguranca_crv: '123123',
    });

    expect(response).toMatchObject({ codigo_seguranca_crv: '123123' });
  });
});
