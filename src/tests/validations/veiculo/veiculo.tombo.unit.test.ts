...
describe.skip('Unit test for veiculo post request', () => {
  ...
  test('should throw an error if tombo is invalid', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);

    try {
      await sut.context.validateData(schema, { tombo: 1 });
    } catch (error) {
      expect(error).toBeInstanceOf(ValidationError);
      expect(error.value).toMatchObject({ tombo: 1 });
      expect(error.errors[0]).toEqual('Formato de Tombo inválido');
    }
  });

  test('should return an object containing tombo if tombo is validated', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);
    ...
    expect(response).toMatchObject({ tombo: 'POI1311' });
  });
});
