...
describe.skip('Unit test for veiculo post request', () => {
  ...
  test('should throw an error if id_marca is not set', async () => {
    sut.context.setStrategy(sut.yupValidation);
    ...
    try {
      ...
      expect(error.errors[0]).toEqual(requiredField('Marca'));
    }
  });

  test('should throw an error if id_marca is invalid', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);

    try {
      await sut.context.validateData(schema, { id_marca: 'auhgr' });
    } catch (error) {
      expect(error).toBeInstanceOf(ValidationError);
      expect(error.value).toEqual({ id_marca: Number.NaN });
      expect(error.errors[0]).toEqual('Campo de marca deve ser um numero');
    }
  });

  test('should return an object containing id_marca if id_marca is validated', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);
    ...
    expect(response).toMatchObject({ id_marca: 1 });
  });
});
