...
describe.skip('Unit test for veiculo post request', () => {
  ...
  test('should throw an error if tipo_doc_carga is invalid', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);

    try {
      await sut.context.validateData(schema, { tipo_doc_carga: '145' });
    } catch (error) {
      expect(error).toBeInstanceOf(ValidationError);
      expect(error.value).toMatchObject({ tipo_doc_carga: '145' });
      expect(error.errors[0]).toEqual('Opção inválida de Tipo Documento Carga');
    }
  });

  test('should return an object containing tipo_doc_carga if tipo_doc_carga is validated', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);
    ...
    expect(response).toMatchObject({ tipo_doc_carga: '0' });
  });
});
