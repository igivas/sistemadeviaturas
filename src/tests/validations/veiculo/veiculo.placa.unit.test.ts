...
describe.skip('Unit test for veiculo post request', () => {
  ...
  test('should throw an error if placa is invalid', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);

    try {
      await sut.context.validateData(schema, { placa: 'asdf' });
    } catch (error) {
      expect(error).toBeInstanceOf(ValidationError);
      expect(error.value).toMatchObject({ placa: 'ASDF' });
      expect(error.errors[0]).toEqual('Placa inválida');
    }
  });

  test('should return an object containing placa if placa is validated', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);
    ...
    expect(response).toMatchObject({ placa: 'POI1311' });
  });
});
