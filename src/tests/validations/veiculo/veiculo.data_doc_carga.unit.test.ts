...
describe.skip('Unit test for veiculo post request', () => {
  ...
  test('should throw an error if data_doc_carga is invalid', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);

    try {
      await sut.context.validateData(schema, { data_doc_carga: 'asdf' });
    } catch (error) {
      expect(error).toBeInstanceOf(ValidationError);

      expect(error.errors[0]).toEqual('Data invalida');
    }
  });

  test('should return an object containing data_doc_carga if data_doc_carga is validated', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);

    const data_doc_carga = '2020-12-31T03:00:00.000Z';

    const expectedDataDocCarga = data_doc_carga;
    ...
    expect(response).toEqual({
      data_doc_carga: new Date(expectedDataDocCarga),
    });
  });
});
