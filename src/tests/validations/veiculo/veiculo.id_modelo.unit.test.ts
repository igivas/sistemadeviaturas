...
describe.skip('Unit test for veiculo post request', () => {
  ...
  test('should throw an error if id_modelo is not set', async () => {
    sut.context.setStrategy(sut.yupValidation);
    ...
    try {
      ...
      expect(error.errors[0]).toEqual(requiredField('Modelo'));
    }
  });

  test('should throw an error if id_modelo is invalid', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);

    try {
      await sut.context.validateData(schema, { id_modelo: 'auhgr' });
    } catch (error) {
      expect(error).toBeInstanceOf(ValidationError);
      expect(error.value).toEqual({ id_modelo: Number.NaN });
      expect(error.errors[0]).toEqual('Campo de modelo deve ser um numero');
    }
  });

  test('should return an object containing id_modelo if id_modelo is validated', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);
    ...
    expect(response).toMatchObject({ id_modelo: 1 });
  });
});
