...
describe.skip('Unit test for veiculo post request', () => {
  ...
  test('should throw an error if data_operacao is invalid', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);

    try {
      await sut.context.validateData(schema, { data_operacao: 'asdf' });
    } catch (error) {
  ...
  test('should return an object containing data_operacao if data_operacao is validated', async () => {
    sut.context.setStrategy(sut.yupValidation);
    const schema = sut.context.createSchema(createVeiculoRules);

    const data_operacao = '2020-12-31T03:00:00.000Z';

    const expectedDataDocCarga = data_operacao;
    ...
    expect(response).toEqual({
      data_operacao: new Date(expectedDataDocCarga),
    });
  });
});
